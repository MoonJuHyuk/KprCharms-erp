...
if menu == "대시보드":
    ...
    if not df_logs.empty:
        ...
        out_val = df_target_day[df_target_day['구분']=='출고']['수량'].sum()
        pend_cnt = len(df_orders[df_orders['상태']=='준비']['주문번호'].unique())
        ...
        filter_opt = c_filter2.selectbox("조회 품목 필터", ["전체", "KA", "KG", "KA반제품", "Compound"])
...
# [1] 재고/생산 관리
    ...
    with st.sidebar:
        ...
        if st.button("저장"):
            if sel_code:
                sheet_logs.append_row([date.strftime('%Y-%m-%d'), datetime.datetime.now().strftime("%H:%M:%S"), factory, cat, sel_code, item_info['품목명'], item_info['규격'], item_info['타입'], item_info['색상'], qty_in, note_in])
